...
{
  _ProcessFolder(inputFolder, &files, &fileCount);
  for(int i = 0; i < fileCount; i++)
  {
    switch (mode)
    {
    case ENCRYPT:
      EncryptCBC(files[i].fileContent, key, iv, outputFolder, _SetOutputFileName(files[i].fileName, mode));
      break;
    case DECRYPT:
      DecryptCBC(files[i].fileContent, key, iv, outputFolder, _SetOutputFileName(files[i].fileName, mode));
      break;
    default:
      printf("Invalid mode. \n");
      break;
...
void HandleInputUsingThread(int mode, char* inputFolder, char* outputFolder, FileInfo* files, int fileCount, uint8_t* key, uint8_t* iv, int num_threads)
{
  ...
  int files_per_thread = (fileCount + num_threads - 1) / num_threads;
  ...
  for(int i = 0; i < num_threads; i++)
  {
    ...
    int end_index = (i+1) * files_per_thread;
    if(end_index > fileCount)
    {
      end_index = fileCount;
    }

    thread_data[i].files = &files[start_index];
    thread_data[i].fileCount = end_index - start_index;
    thread_data[i].mode = mode;
    thread_data[i].key = key;
    thread_data[i].iv = iv;
    thread_data[i].outputFolder = outputFolder;

    pthread_create(&threads[i], NULL, _ProcessFilesUsingThread, (void*)&thread_data[i]);
  }
...
void* _ProcessFilesUsingThread(void* arg)
{
  ...
  uint8_t* iv = data->iv;
  char* outputFolder = data->outputFolder;
  int mode = data->mode;
...
void EncryptCBC(char* plainText, uint8_t* key, uint8_t* iv, char* outputFolder, char* outputFileName)
{
  ...
  _ExportFile(hexarray, dlenu, outputFolder, outputFileName, "-e");
}

void DecryptCBC(char* cipherText, uint8_t* key, uint8_t* iv, char* outputFolder, char* outputFileName)
{

  int dlen = strlen(cipherText)/2;
  int klen = strlen(key);

  //Proper Length of cipher text
  int dlenu = dlen;
  if (dlen % 16) {
      dlenu += 16 - (dlen % 16);
  }
  ...
  // Make the uint8_t arrays using dynamic allocation
  uint8_t *hexarray = (uint8_t *)malloc(dlenu * sizeof(uint8_t));
  uint8_t kexarray[klenu];
  memset( hexarray, 0, dlenu );
  memset( kexarray, 0, klenu );
  ...
  // Fill the uint8_t arrays
  hexarray = _HexStringToByteArray(cipherText);
  int keyPad = pkcs7_padding_pad_buffer( kexarray, klen, sizeof(kexarray), 16 );

  /*printf("The padded key in hex is = ");
  for (uint8_t i=0; i<klenu;i++){
      printf("%02x",kexarray[i]);
  }
  printf("\n");*/
  ...
  /*printf("Hexarray after init: ");
  for(int i=0;i<dlen;i++)
  {
    printf("%02X ", hexarray[i]);
  }
  printf("\n");
  printf("iv in decrypt: ");
  for (size_t i = 0; i < iv_len; i++) {
      printf("%02X ", iv[i]);
  }
  printf("\n");*/
  ...
  for (uint8_t i=0; i<actualDataLength;i++){
      printf("%02x",hexarray[i]);
  }
  printf("\n");*/

  //size_t hex_length = sizeof(hexarray) / sizeof(hexarray[0]);
  //hex_to_char(hexarray, hex_length);
  _ExportFile(hexarray, dlenu, outputFolder, outputFileName, "-d");
